        ...
        for j in range(len(time_dict.values()[i].keys())): # time_dict.values()[0].keys() times in the dict
            ...
            if formatted_time in formatted_dict:
                ...
                formatted_dict[formatted_time] += ((int(cur_times[1]) - int(pre_times[1])) * \
                                                  time_dict.values()[i].values()[j-1]) / screen_size
                print formatted_dict
            else:
                formatted_dict[formatted_time] = time_dict.values()[i].values()[j] * \
                                                 (int(cur_times[1]) - formatted_min)
                ...
                if formatted_dict.keys().index(formatted_time) != 0:
                    ...
                    formatted_dict[formatted_dict.keys()[formatted_dict.keys().index(formatted_time) - 1]] += \
                        (upper_gap * time_dict.values()[i].values()[j]) / screen_size
                    ...
                    # print(time)
                    print formatted_dict
        daily_power[i] = daily_power.append(time_dict.keys()[i])
        daily_power[i] = daily_power.append(formatted_dict)
        print daily_power
